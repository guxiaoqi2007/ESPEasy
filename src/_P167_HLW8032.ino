...
#define PLUGIN_NAME_167       "Energy (AC) - HLW8032"
#define PLUGIN_VALUENAME1_167 "Voltage"       //电压
#define PLUGIN_VALUENAME2_167 "Current"       //电流
#define PLUGIN_VALUENAME3_167 "Power"         //功率
// #define PLUGIN_VALUENAME4_167 "Total Pulses"  //脉冲总数量
#define PLUGIN_VALUENAME5_167 "Energy"        //电量
...
boolean Plugin_167(byte function, struct EventStruct *event, String &string) {
  ...
  switch (function) {
    ...
    case PLUGIN_GET_DEVICEVALUENAMES: {
      ...
      // (ExtraTaskSettings.TaskDeviceValueNames[3], PSTR(PLUGIN_VALUENAME4_167));
      strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[3], PSTR(PLUGIN_VALUENAME5_167));
      break;
    }
    ...
    case PLUGIN_INIT: {
      initPluginTaskData(event->TaskIndex, new P167_data_struct());
    ...
    case PLUGIN_SERIAL_IN: {
      P167_data_struct* P167_data = static_cast<P167_data_struct*>(getPluginTaskData(event->TaskIndex));
      if (nullptr != P167_data) {
        success = true;
        if (P167_data->processSerialData()) {     //校验数据
          CseReceived(event);
          //更新数据显示
          UserVar[event->BaseVarIndex] = P167_data->energy_voltage;
          UserVar[event->BaseVarIndex + 1] = P167_data->energy_current;
          UserVar[event->BaseVarIndex + 2] = P167_data->energy_power;
          // UserVar[event->BaseVarIndex + 3] = P167_data->total_cf_pulses;
          UserVar[event->BaseVarIndex + 3] = P167_data->energy;
        }
      }
      break;
    }
...
bool CseReceived(struct EventStruct *event) {
  ...
  if (!P167_data->processCseReceived(event)) {
    return false;
  }
  return true;
}
